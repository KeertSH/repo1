...
public class ParallelDBWriter {

	private static Logger logger = LogManager.getLogger( ParallelDBWriter.class );

	private final int nThreads;
	private DBWriter[] writers;
	private Thread[] threads;

	public ParallelDBWriter( int nThreads ) {
	...
	public void write( CompleteSubscriber row ) throws InterruptedException {
		int turn = Integer.parseInt( row.getMsisdn().substring( 6 ) ) % nThreads;
		writers[turn].write( row );
	}
	...
	private static class DBWriter implements Runnable, AutoCloseable {
		...
		private final BlockingQueue<CompleteSubscriber> queue;
		...
		private void write( CompleteSubscriber row ) throws InterruptedException {
			queue.put( row );
		}

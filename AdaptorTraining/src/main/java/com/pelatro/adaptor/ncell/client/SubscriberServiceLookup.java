...
public class SubscriberServiceLookup implements Publisher, ResponseHook, CancelHook {

	private static final Logger logger = LogManager.getLogger( SubscriberServiceLookup.class );
	...
	private static final String malformedMsisdn = "0000000000000";
	...
	public SubscriberServiceLookup( ServiceCallback callback ) {
		super();
	...
	public void publish( Iterable<Context> volume ) {
		synchronized (requestLock) {
			...
			for ( Context context : volume ) {
				...
				msisdn = msisdn.length() == 13 ? msisdn : malformedMsisdn;
